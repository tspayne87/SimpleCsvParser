...
namespace SimpleCsvParser.Test
{
    ...
    public class DataParserTests
    {
        ...
        {
            var parser = new CsvParser(new CsvParserOptions() { Delimiter = ';', Wrapper = '*' });
        ...
        {
            var parser = new CsvParser(new CsvParserOptions() { ParseHeaders = false });
        ...
        {
            var parser = new CsvParser(new CsvParserOptions() { AllowDefaults = false });
            var result = parser.Parse<TestModel>("name,type,cost,id,date\nClaws,Attachment,10,\"34.5\",03/27/1987").FirstOrDefault();
